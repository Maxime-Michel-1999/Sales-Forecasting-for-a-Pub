...
CREATE TABLE `High_Degree_Beer` 
	AS SELECT babar_server_purchase.* FROM babar_server_purchase JOIN babar_server_product ON babar_server_purchase.product_id = babar_server_product.id WHERE
	YEAR(TIMESTAMP)>2013 AND YEAR(TIMESTAMP) <2018 AND 
...
CREATE TABLE `Normal_Degree_Beer` 
	AS SELECT babar_server_purchase.* FROM babar_server_purchase JOIN babar_server_product ON babar_server_purchase.product_id = babar_server_product.id WHERE
	YEAR(TIMESTAMP)>2013 AND YEAR(TIMESTAMP) <2018 AND 
